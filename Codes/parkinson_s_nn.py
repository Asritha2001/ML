# -*- coding: utf-8 -*-
"""Parkinson's_NN.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1u1sOpMV3UbcGcPS-bXg1VQyeJS4P-EL9
"""
...
def evaluate_model_on_training(X,onehot_labels,layers):
  ...
  plt.ylabel('Performance of cost (J) on the test set')
...
def evaluate_model(X_train, y_train, X_test, y_test,layers) :
  ...
  N = NeuralNet(layers, regularization_param)
  for i in range(epochs):
    N.back_prop(X_train, y_train, learning_rate)
  a = N.forward_prop(X_test)
  y_pred = a[-1]
  J = N.cost_instance(y_pred,y_test)
  accuracy, f1_score = calculate_accuracy_f1score(y_test, y_pred)
  return J,accuracy,f1_score

def train(x,y):
  X = np.array(normalize(x))
  unique_values = np.unique(np.array(y), axis=0)
  onehot_labels = []
  for i in range(len(y)):
    y_i = [0]*len(unique_values)
    for j in range(len(unique_values)):
        if y[i] == unique_values[j]:
            y_i[j] = 1
    onehot_labels.append(y_i)
  onehot_labels = np.array(onehot_labels)
  input = X.shape[1]
  output = onehot_labels.shape[1]
  layers=[input,4,3, output]
  kfolds = 10
...
df = pd.read_csv('parkinsons.csv', sep=',', skiprows = 1, header=None)
X = df.iloc[:, :-1].values
